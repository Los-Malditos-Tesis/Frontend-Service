...
const AnalysisPanel = ({
  analysis,
  renderResultIcon,
  renderStatusCardStyle,
  renderAnalysisCameraCard,
  getLocationName,
}) => {
  return (
    <Box
      ...
      {analysis ? (
        <Box sx={{ mt: 2, display: "grid", gap: 2 }}>
          ...
          {analysis.primaryMatch && (
            ...
            >
              <Typography variant="subtitle2" sx={{ fontWeight: 950, mb: 1.5, color: "#0f172a" }}>
                Detección principal
              </Typography>
          ...
          {analysis.matchedCameras?.length > 0 && (
            ...
            >
              ...
              >
                {analysis.matchedCameras.map((entry, index) =>
                  renderAnalysisCameraCard(entry, index, "matched")
                )}
              </Box>
            </Box>
          )}
            ...
            >
              {analysis.respondedCameras?.length > 0 && (
                ...
                >
                  <Typography
                    variant="subtitle2"
                    sx={{ fontWeight: 950, mb: 1.5, color: "#0f172a" }}
                  >
                    Cámaras que respondieron
                  </Typography>
                  <Box sx={{ display: "grid", gap: 1 }}>
                    {analysis.respondedCameras.map((entry, index) =>
                      renderAnalysisCameraCard(entry, index)
                    )}
                  </Box>
                </Box>
              )}
              ...
              {analysis.notRespondedCameras?.length > 0 && (
                ...
                >
                  <Typography
                    ...
                    sx={{ fontWeight: 950, mb: 1.5, color: "#0f172a" }}
                  >
                    Cámaras sin respuesta
                  </Typography>
                  <Box sx={{ display: "grid", gap: 1 }}>
                    {analysis.notRespondedCameras.map((entry, index) =>
                      renderAnalysisCameraCard(entry, index)
                    )}
                  </Box>
                </Box>
          ...
          >
            ...
            <Typography variant="body2" sx={{ color: "#475569", fontWeight: 700 }}>
              Categoría analizada: {analysis.productCategory || "--"}
            </Typography>
            {analysis.scannedAt && (
              <Typography variant="body2" sx={{ color: "#475569", mt: 0.5, fontWeight: 700 }}>
                Escaneado en: {analysis.scannedAt}
              </Typography>
            )}
